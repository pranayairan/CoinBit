...
class IntroPageTransformer : ViewPager.PageTransformer {
    ...
    override fun transformPage(page: View, position: Float) {
        ...
        if (position <= -1.0f || position >= 1.0f) {
            ...
            val description = page.findViewById<TextView>(R.id.tvSubTitle)
            description.translationX = (-(1 - position) * 0.2 * pageWidthTimesPosition).toFloat()
            ...
            val animation = page.findViewById<LottieAnimationView>(R.id.animationView)
            animation.translationX = ((-(1 - position) * .7 * pageWidthTimesPosition)).toFloat()

            val button = page.findViewById<Button>(R.id.btnChooseCurrency)
            animation.translationX = (-(-(1 - position) * .7 * pageWidthTimesPosition)).toFloat()
        }
    }
}
